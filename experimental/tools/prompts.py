...
vds_schema = {
    "FieldBase": {
        ...
        "properties": {
            ...
            # NEW: optional logicalTableId to disambiguate duplicate captions across logical tables
            "logicalTableId": {
                "type": "string",
                "description": "Optional; used to disambiguate the same caption across multiple logical tables."
            },
            "maxDecimalPlaces": {
                "type": "integer",
                "description": "The maximum number of decimal places. Any trailing 0s will be dropped. The maxDecimalPlaces value must be greater or equal to 0."
    ...
    "FieldMetadata": {
        ...
        "properties": {
            ...
            "dataType": {
                "type": "string",
                "enum": ["INTEGER","REAL","STRING","DATETIME","BOOLEAN","DATE","SPATIAL","UNKNOWN"]
            },
            "logicalTableId": { "type": "string" }
        }
    },
    "Filter": {
        "type": "object",
        ...
        "required": ["field", "filterType"],
        "properties": {
            "field": { "$ref": "#/components/schemas/FilterField" },
            "filterType": {
                "type": "string",
                "enum": ["QUANTITATIVE_DATE","QUANTITATIVE_NUMERICAL","SET","MATCH","DATE","TOP"]
            },
            "context": {
                "type": "boolean",
                "description": "Make the given filter a Context Filter, meaning that it's an independent filter. Any other filters that you set are defined as dependent filters because they process only the data that passes through the context filter",
        ...
        "oneOf": [
            {
                ...
                "properties": {
                    ...
                    # NEW: optional LTID to disambiguate in filters
                    "logicalTableId": {
                        "type": "string",
                        "description": "Optional; to disambiguate a caption across logical tables"
                    }
                }
            },
            {
    ...
    "MatchFilter": {
        "allOf": [
            { "$ref": "#/components/schemas/Filter" },
            {
                ...
                "description": "A Filter that can be used to match against String Fields",
                "properties": {
                    "contains": { "type": "string", "description": "Matches when a Field contains this value" },
                    "startsWith": { "type": "string", "description": "Matches when a Field starts with this value" },
                    "endsWith": { "type": "string", "description": "Matches when a Field ends with this value" },
                    "exclude": {
                        "type": "boolean",
                        "description": "When true, the inverse of the matching logic will be used",
                        "default": False
    ...
    "QuantitativeNumericalFilter": {
        "allOf": [ { "$ref": "#/components/schemas/QuantitativeFilterBase" } ],
        "type": "object",
        "description": "A Filter that can be used to find the minimumn, maximumn or range of numerical values of a Field",
        "properties": {
            "min": {
            ...
            { "$ref": "#/components/schemas/Filter" },
            {
                ...
                "properties": {
                    ...
                    "dateRangeType": {
                        "type": "string",
                        "description": "The direction in the relative date range",
                        "enum": ["CURRENT","LAST","LASTN","NEXT","NEXTN","TODATE"]
                    },
                    "rangeN": {
                        "type": "integer",
                        "description": "When dateRangeType is LASTN or NEXTN, this is the N value (how many years, months, etc.)."
    ...
    "TopNFilter": {
        ...
        "type": "object",
        "description": "A Filter that can be used to find the top or bottom number of Fields relative to the values in the fieldToMeasure",
        "required": ["howMany", "fieldToMeasure"],  # fixed
        "properties": {
            "direction": {
                "type": "string",
                "enum": ["TOP", "BOTTOM"],
            ...
            "fieldToMeasure": { "$ref": "#/components/schemas/FilterField" }
        }
    }
}
